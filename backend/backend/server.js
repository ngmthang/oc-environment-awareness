...
// If your frontend is served by THIS SAME Express app (recommended), you can keep origin:true.
// If you later host frontend somewhere else, set FRONTEND_URL to that exact origin.
app.use(
  cors({
    origin: true,
    credentials: true,
...
app.use(
  session({
    ...
    cookie: {
      secure: process.env.NODE_ENV === "production", // Render is HTTPS so this is fine in prod
      httpOnly: true,
...
// Ensure your repo has: public/login.html, public/main.html, public/css/styles.css, public/js/...
app.use(express.static(path.join(__dirname, "public")));

// Always land on login page at /
app.get("/", (req, res) => {
  ...
  const [
    ...
    pool.query(
      `SELECT COUNT(DISTINCT v.id) AS count
        ...
        WHERE vi.created_at >= DATE_TRUNC('day', NOW() AT TIME ZONE $1) AT TIME ZONE $1
          AND vi.created_at <  (DATE_TRUNC('day', NOW() AT TIME ZONE $1) + INTERVAL '1 day') AT TIME ZONE $1
          AND v.role = 'GUEST'`,
      [tz]
    ),
    pool.query(
...
app.post("/api/quiz/submit", requireSession, async (req, res) => {
  try {
    ...
    if (Number.isNaN(score) || score < 0 || score > total) {
      return res.status(400).json({ error: "Invalid score" });
    }

    const { rows } = await pool.query(`SELECT * FROM visitors WHERE id = $1`, [
    ...
    if (score > visitor.quiz_best_score) {
      ...
      await pool.query(`UPDATE visitors SET quiz_completed = TRUE WHERE id = $1`, [
        visitor.id,
      ]);
    }
